...
def get_client_ip():
    # Check if the request has the 'X-Forwarded-For' header, which is set by proxies
    if request.headers.get('X-Forwarded-For'):
        # It may contain a list of IPs, so we take the first one
...
def get_ip_info():
    # Get the IP address from the request arguments, or use the detected client's IP
